...
Artifacts may be unsigned; signing is optional.
Checksum verification is required.

When a Windows artifact is signed, verify that `Zarya.exe` reports
`SignPath Foundation` as the Authenticode publisher.
